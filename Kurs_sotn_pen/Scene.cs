	...
	namespace Kurs_sotn_pen
{
    ...
    public class Scene
    {
        ...
        public PenObject SelectedForPanObject = null;
        public bool IsPanning { get; set; }
        public Vector3D Light { get; set; }
        public Camera Camera { get; set; }
        public MODE Mode { get; set; } = MODE.WIREFRAME;
        public List<PenObject> Objects { get; private set; } = new List<PenObject>();
        ...
        public void AddObject(PenObject obj)
        {
            Objects.Add(obj);
        }

        public PenObject GetObjectByName(string name)
        {
            foreach (PenObject obj in Objects)
            {
        ...
        public void DrawObjects()
        {
            ...
            else
            {
                ...
                {
                    SceneObject PenObject = Objects[i];
                    if (IsVisibleForCamera(PenObject.ObjectBasePoint, PenObject.MaxLength, cameraMatrix))
                    {
                        DrawObject(PenObject, bitmap, buffer, cameraMatrix);
                    }
                }
            }
        ...
        /// </summary>
        private void DrawObject(SceneObject PenObject, FastBitmap bitmap, double[,] buffer, Matrix cameraMatrix)
        {
            // how basic object coordinates are modified (basePoint)
            Matrix objectMatrix = PenObject.NewModificationMatrix();
            foreach (PrimitiveWithName scenePrimitive in PenObject.Primitives)
            {
                ...
                foreach (Face face in scenePrimitive.Primitive.Faces)
                {
                    ...
                    { // then draw all the lines, the order is non-important
                        DrawObjectLines(bitmap, buffer, points, scenePrimitive.Primitive.Color, primitiveMatrix, scenePrimitive.Primitive.BasePoint, objectMatrix, PenObject.ObjectBasePoint, cameraMatrix);
                    }
                    else
                    {
                        // convert object primitive coordinates to world coords.
                        ApplyObjectMatrix(points, primitiveMatrix, scenePrimitive.Primitive.BasePoint, objectMatrix, PenObject.ObjectBasePoint);
        ...
        private void DrawLine(FastBitmap bitmap, double[,] buffer, Point3D start, Point3D end, Color color)
        {
            ...
            if (Math.Abs(diffx) > Math.Abs(diffy))
            {
                ...
                double[] yCoords = MathHelpers.Interpolate(start.X, start.Y, end.X, end.Y);
                double[] zCoords = MathHelpers.Interpolate(start.X, start.Z, end.X, end.Z);
            ...
            else
            {
                ...
                double[] xCoords = MathHelpers.Interpolate(start.Y, start.X, end.Y, end.X);
                double[] zCoords = MathHelpers.Interpolate(start.Y, start.Z, end.Y, end.Z);
